# Changelog

## Unreleased

### Added

- Dalen has a page and a card, in beta, with its new painted icon.
- Lune has a page with its screenshots and a TestFlight beta status.
- Tide has a coming soon page.
- Fork's page shows its first App Store review.

### Changed

- The homepage apps are in three tiers. Apps on the App Store get large feature
  cards, with a glow taken from each icon, the name in the wordmark's serif and
  an "On the App Store" line. Beta apps are compact cards with a status dot, and
  coming-soon apps are a quiet row of icons that only become cards on hover. The
  cards arrive one after another.
- Bins is marked live, links to its App Store page, and shows cleaner screenshots.
- Keel's gallery uses the six current 1.1.0 framed screenshots.
- The homepage apps are separate App Store-style cards: real app icons, names
  and a capsule showing whether each app is out, in beta, in review or coming
  soon. Cards lift gently on hover and settle when pressed.
- Fork and Keel use the icon artwork from their App Store listings.
- Fork's page has the 1.1.0 screenshots and describes bass, ukulele and 432 Hz.
- App pages use each app's real icon where one exists.
- llms.txt and llms-full.txt list Lune and Tide, Fork's new instruments and
  Bins on the App Store.
- The homepage lists Fork, Keel and Bins first, then Lune, Enso, Tide and the
  coming soon apps.
- Simplify the README to development, project structure, analytics and licensing.
- Remove internal workflow narratives, private source paths and undated
  performance claims.
